...
class NewEvent(FlaskForm):
    ...
    new_event_type_of_emergency_services_needed = SelectField("Forward event to:", choices=["Straż Pożarna", "Policja", "Ratownictwo medyczne"])
    new_event_caller_telephone_number = IntegerField("Telephone number:", validators=[data_required(), length(min=9)])
    new_event_caller_name = StringField()
    new_event_caller_surname = StringField()
